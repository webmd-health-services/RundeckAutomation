...
Describe ('RundeckAutomation.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_RundeckAutomation' -Exists
    }
}

Describe ('RundeckAutomation.command verbs') {
...
Describe ('RundeckAutomation.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'RundeckAutomation' -CommandType Function,Cmdlet,Filter))
        {
